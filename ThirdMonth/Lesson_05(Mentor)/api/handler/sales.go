...
func (h *Handler) GetSale(c *gin.Context) {
	sales := models.Sales{}
	id := c.Param("id")
	ok, err := h.redisStorage.Cache().Get(c.Request.Context(), id, &sales)
	if err != nil {
...
func (h *Handler) UpdateSale(c *gin.Context) {
	...
	err = h.redisStorage.Cache().Delete(c.Request.Context(), sales.Id)
	if err != nil {
		fmt.Println("error Sales  Delete in redis cache:", err.Error())
	}
