...
func (h *Handler) GetTransaction(c *gin.Context) {
	...
	id := c.Param("id")
	ok, err := h.redisStorage.Cache().Get(c.Request.Context(), id, &transaction)
	if err != nil {
	...
	if err != nil {
		fmt.Println("error Transaction Create in redis cache:", err.Error())
	}
}
...
func (h *Handler) UpdateTransaction(c *gin.Context) {
	...
	err = h.redisStorage.Cache().Delete(c.Request.Context(), Transaction.Id)
	if err != nil {
		fmt.Println("error Transaction  Delete in redis cache:", err.Error())
	}
}
